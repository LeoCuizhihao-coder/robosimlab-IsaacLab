...
def project_world_frame_point_to_camera_2d_plane(eef_pose_w, intrinsic, extrinsic):
    ...
    ee_pos_w = eef_pose_w[:, 0:3].squeeze()  # ee pose position in world frame
    ee_matrix_w = eef_pose_w[:, 3:6].squeeze()  # ee pose quat in world frame
...
def main():
    dataset_path = r'D:\git_project\IsaacLab\logs\robomimic\Isaac-Lift-Cube-Franka-IK-Rel-dp_20240913_simple3'
    ckpt_path = r"D:\git_project\IsaacLab\logs\robomimic\Isaac-Lift-Cube-Franka-IK-Rel-dp_20240913_simple3\epoch=0100-train_loss=0.003.ckpt"
    ...
    gripper_position = in_replay_buffer.data['gripper_position'][:]
    ...
    for episode_idx, episode_length in enumerate(episode_lengths):
        ...
        print("episode_video_paths ", episode_video_paths)
        ...
        print("[Info] view_1 ", view_1)
        print("[Info] view_3 ", view_3)
        print("[Info] episode_length ", episode_length)
        ...
        for step_idx, (camera_1_image, camera_3_image) in enumerate(
            ...
            gripper_pos = gripper_position[frame_id]

            # print("robot_eef_position ", robot_eef_pose)
            # print("robot_gripper_width ", gripper_pos)
            # print("camera_3_image ", camera_3_image)
            # print("camera_3_image.shape ", camera_3_image.shape)

            camera_1_arr.append(camera_1_image)
            camera_3_arr.append(camera_3_image)  # [240, 320, 3]
            eef_arr.append(robot_eef_pose)
            gripper_arr.append(gripper_pos)
            ...
            if len(camera_1_arr) == 2 and len(camera_3_arr) == 2 and len(eef_arr) == 2:
                obs = {
                    "camera_1": np.array(camera_1_arr),  # [2, H, W, 3]
                    "camera_3": np.array(camera_3_arr),  # [2, H, W, 3]
                    "robot_eef_pose": np.array(eef_arr),  # [2, 6]
                    "gripper_position": np.array(gripper_arr),  # [2, 1]
                }
                ...
                with torch.no_grad():
                    ...
                    assert action.shape[-1] == 2 or action.shape[-1] == 6 or action.shape[-1] == 7
                    ...
                    gripper_width = first_action[:, 6:7]
                    __first_action = torch.concat([first_action[:, 0:3], robot_eef_pose[:, 3:6]], dim=-1)
                    pred_action_pos = project_world_frame_point_to_camera_2d_plane(__first_action,
                                                                                   intrinsic=tv_intrinsic,
                                                                                   extrinsic=tv_extrinsic)
                    print("gripper_width ", gripper_width)
                    # openCV read rgb as bgr, therefore needs transform.
                    camera_3_image = cv2.cvtColor(camera_3_image, cv2.COLOR_BGR2RGB)
